// 🔷 node.js 의 내장모듈 
    // node.js 에서 미리 만들어놓은 '내장 모듈' 
    // require 에서 '모듈 경로 & 파일명' 적는게 아님
    // '모듈의 이름만!' 적기
...
// 🔷 운영체제의 정보가 들어있는 모듈
    ...
    console.log(os)
        // [해석]
            // 미리 만들어둔 모듈을 쓸 때는 '이름만!' 쓰면 됨. 
            // 서버 열고 할 때, os가 들어가게 됨. 


// 🔷 node.js 의 내장 객체 
    // node의 전역객체는 global 임 (브라우저에서 보던 winddow 처럼)
    // 브라우저와 노드는 ⭐런타임 환경⭐ 이 다르기 때문에 ⭐전역 객체⭐ 도 다르다.
    // 노드의 전역객체는 ⭐global⭐ 임. 

    // js 에서 this 를 작성하고 확인하면, module.exports 가 뜬다. 
    // 참조가 들어가기 때문에, 다르지 않을까? 
        // 본인을 참조시키기 때문에, exports 가 뜨는 것 같음?
        // 하지만, 전역 객체는 global
        // global 객체는 개념만 알아두기 ❓❓❓❓❓ 
